...
const Weather = () => {
  ...
  useEffect(() => {
    setquery("");
    fetch(
      `https://api.weatherapi.com/v1/forecast.json?key=${import.meta.env.VITE_API_KEY
  ...
  useEffect(() => {
    ...
    if (query) {
      fetch(
        `https://api.weatherapi.com/v1/search.json?key=${import.meta.env.VITE_API_KEY
        }&q=${query}}`
      )
        .then((res) => res.json())
        .then((d) => setresult(d));
  ...
  return data ? (
    <div className="flex h-screen flex-col lg:flex-row">
      <div className="relative bg-gradient-to-l from-[#53bfc9] to-cyan-400 lg:w-1/2 h-full flex flex-col justify-center p-16 text-white">
        ...
        <svg className="absolute bottom-0 left-0  opacity-60" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1440 320">
          <path fill="#2d91d4" fill-opacity="1" d="M0,288L48,272C96,256,192,224,288,197.3C384,171,480,149,576,165.3C672,181,768,235,864,250.7C960,267,1056,245,1152,250.7C1248,256,1344,288,1392,304L1440,320L1440,320L1392,320C1344,320,1248,320,1152,320C1056,320,960,320,864,320C768,320,672,320,576,320C480,320,384,320,288,320C192,320,96,320,48,320L0,320Z"></path>
        </svg>
      </div>
              ...
              <span className="flex flex-row justify-center items-center gap-2">
                <span>
                  <Icon
                    ...
                    style={{
                      transform: `rotate(${data.current.wind_degree - 90}deg)`,
                    }}
                  />
                </span>
